...
from .models import Statement, Deposit, Withdrawal, Notesdb, Strategy, Buy_calc, Quarter, Pipmargin
from .forms import DepositForm, WithdrawalForm, NotesdbForm, StrategyForm, Buy_calcForm, QuarterForm, PipmarginForm
import pandas as pd
import json, time
...
def strategy(request):
    strat = Strategy.objects.all().values()
    db_strategy = pd.DataFrame(strat)
    pip = Pipmargin.objects.all().values()
    db_pip = pd.DataFrame(pip)
    filt_pip = (db_pip['market'] == "US100")
    pip_us100 = float(db_pip.loc[filt_pip, 'pip'])
    margin_us100 = int(db_pip.loc[filt_pip, 'margin'])
    # *** Get REAL value ***
    wall_real = wallet()
    filt_strategy = (db_strategy['id'] == 1)
    point_real = int(db_strategy.loc[filt_strategy, 'point_real'])
    percent_real = float(db_strategy.loc[filt_strategy, 'percent_real'])
    # *** Get VIRTUAL value ***
    filt_strategy = (db_strategy['id'] == 2)
    wall_virtual = int(db_strategy.loc[filt_strategy, 'virtual_depo'])
    point_virtual = int(db_strategy.loc[filt_strategy, 'point_virtual'])
    percent_virtual = float(db_strategy.loc[filt_strategy, 'percent_virtual'])
    # *** Get LONG TERM value ***
    filt_strategy = (db_strategy['id'] == 3)
    depo_year = int(db_strategy.loc[filt_strategy, 'depo_year'])
    percent_year = float(db_strategy.loc[filt_strategy, 'percent_year'])
    years_year = int(db_strategy.loc[filt_strategy, 'years_year'])
    months_year = int(db_strategy.loc[filt_strategy, 'months_year'])
    days_year = int(db_strategy.loc[filt_strategy, 'days_year'])
    # *** Get Multiplying the deposit and withdrawal in the month ***
    # *** ===> compound interest profit
    filt_strategy = (db_strategy['id'] == 4)
    depo_month = int(db_strategy.loc[filt_strategy, 'depo_month'])
    percent_month = float(db_strategy.loc[filt_strategy, 'percent_month'])
    wd_month = int(db_strategy.loc[filt_strategy, 'wd_month'])
    # *** Get Multiplying the deposit and withdrawal in the month ***
    # *** ===> constant profit (cp)
    filt_strategy = (db_strategy['id'] == 5)
    depo_month_cp = int(db_strategy.loc[filt_strategy, 'depo_month_cp'])
    profit_month_cp = int(db_strategy.loc[filt_strategy, 'profit_month_cp'])
    wd_month_cp = int(db_strategy.loc[filt_strategy, 'wd_month_cp'])
    # *** Calculate REAL value ***
    profit_real = (wall_real * percent_real) / 100
    vol_real = (profit_real / point_real) / pip_us100 / 100
    margin_real = (vol_real * 100) * margin_us100
    percent_depo_real = (margin_real / wall_real) * 100
    # *** Calculate VIRTUAL value ***
    profit_virtual = (wall_virtual * percent_virtual) / 100
    vol_virtual = (profit_virtual / point_virtual) / pip_us100 / 100
    margin_virtual = (vol_virtual * 100) * margin_us100
    percent_depo_virtual = (margin_virtual / wall_virtual) * 100
    # *** Calculate LONG TERM value ***
    numb_sessions = (years_year * 264) + (months_year * 22) + days_year
    profit_year = depo_year * pow(1 + (percent_year / 100), numb_sessions) - depo_year
    profit_depo = depo_year * pow(1 + (percent_year / 100), numb_sessions)
    # *** Calculate Multiplying the deposit and withdrawal in the month ***
    # *** ===> compound interest profit
    ...
    wd_zip = list(zip(mth, earning, savings, start_next_mth))
    # *** ===> constant profit (cp)
    start_next_mth_cp = []
    earning_cp = profit_month_cp * 22
    savings_cp = earning_cp - wd_month_cp
    start_next_mth_cp.append(savings_cp + depo_month_cp)

    for i in range(13):
        i += 1
        start_next_mth_cp.append(savings_cp + start_next_mth_cp[i - 1])
    wd_cp_zip = list(zip(mth, start_next_mth_cp))
    context = {'wallet_real': wall_real, 'point_real': point_real, 'percent_real': percent_real,
               'profit_real': profit_real, 'vol_real': vol_real, 'margin_real': margin_real,
               'percent_depo_real': percent_depo_real, 'wallet_virtual': wall_virtual, 'point_virtual': point_virtual,
               'percent_virtual': percent_virtual, 'profit_virtual': profit_virtual, 'vol_virtual': vol_virtual,
               'margin_virtual': margin_virtual, 'percent_depo_virtual': percent_depo_virtual, 'depo_year': depo_year,
               'percent_year': percent_year, 'years_year': years_year, 'months_year': months_year,
               'days_year': days_year, 'numb_sessions': numb_sessions, 'profit_year': profit_year,
               'profit_depo': profit_depo, 'depo_month': depo_month, 'percent_month': percent_month,
               'wd_month': wd_month, 'wd_zip': wd_zip, 'profit_month_cp': profit_month_cp,
               'depo_month_cp': depo_month_cp, 'earning_cp': earning_cp, 'wd_month_cp': wd_month_cp,
               'savings_cp': savings_cp, 'wd_cp_zip': wd_cp_zip}
    return render(request, 'cfd/strategy.html', context)


@login_required
def strategy_set(request, id):
    strat = Strategy.objects.all().values()
    df = pd.DataFrame(strat)
    if not df.empty:
        strat_db = Strategy.objects.get(id=id)
        if request.method != 'POST':
            form = StrategyForm(instance=strat_db)
        else:
            form = StrategyForm(instance=strat_db, data=request.POST)
            if form.is_valid():
                form.save()
                return redirect('cfd:strategy')
        return redirect('cfd:strategy')
    else:
        form = StrategyForm(data=request.POST)
        if form.is_valid():
            form.save()
            return redirect('cfd:strategy')
    return render(request, 'cfd/strategy.html')
