...
filt = (df['id'] == 3)
percent_1 = float(df.loc[filt, 'percent_1'])
percent_2 = float(df.loc[filt, 'percent_2'])
percent_3 = float(df.loc[filt, 'percent_3'])

perc = [percent_1, percent_2, percent_3]
# perc = list(zip(percent_1, percent_2, percent_3))
i=0
for per in perc:
    i += 1
    print(per)
